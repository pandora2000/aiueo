...
let rec fv = function (* 式に出現する（自由な）変数 (caml2html: knormal_fv) *)
  | Unit | Int(_) | Float(_) | ExtArray(_) | ExtTuple(_) | Ri | Rf -> S.empty
  | Neg(x) | FNeg(x) | Floor(x) | Float_of_int(x) | Pc x | Pf x -> S.singleton x
...
let rec g env = function (* K正規化ルーチン本体 (caml2html: knormal_g) *)
  ...
  | Syntax.Pc(e) ->
      insert_let (g env e)
	(fun x -> Pc(x), Type.Unit)
  | Syntax.Pf(e) ->
      insert_let (g env e)
	(fun x -> Pf(x), Type.Unit)
  | Syntax.Ri(e) -> Ri, Type.Int
  | Syntax.Rf(e) -> Rf, Type.Float
  | Syntax.Neg(e) ->
      insert_let (g env e)
...
let rec bf e =
  match e with
    ...
    | Syntax.Pc(e) ->Syntax.Pc(bf e)
    | Syntax.Pf(e) ->Syntax.Pf(bf e)
    | Syntax.Ri(e) ->Syntax.Ri(bf e)
    | Syntax.Rf(e) ->Syntax.Rf(bf e)
