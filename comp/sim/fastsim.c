...
#define PARSE_INST_11(INST,INST_NUM)             \
  else if(strcmp (iname,INST) == 0){		\
  answer -> insts[i].name[0] = INST_NUM;	\
  answer -> insts[i].name[1] = int_of_regi(ist.name[1]); \
  }

#define PARSE_INST_12(INST,INST_NUM)             \
  else if(strcmp (iname,INST) == 0){		\
  answer -> insts[i].name[0] = INST_NUM;	\
  answer -> insts[i].name[1] = int_of_fregi(ist.name[1]); \
  }
...
program2 *parse_all2(program * program)
{
  ...
  for (i = 0; i < program->inst_count; i++) {
    ...
    PARSE_INST_1("add", 1)
      ...
      PARSE_INST_11("rdi", 11)
      PARSE_INST_12("rdf", 12)
      PARSE_INST_11("ptc", 13)
      PARSE_INST_12("ptf", 14)
  ...
  while (1) {
    ...
    */
    ...
    DO_INST_1(1, +)
      ...
      print_char(regist[ist.name[1]]);
    }
    else if (iname == 14) {
      print_float(freg[ist.name[1]]);
    }
    /*FPU命令 */
    else if (iname == 15) {
