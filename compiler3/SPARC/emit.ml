...
and g' e =(* 各命令のアセンブリ生成 (caml2html: emit_gprime) *)
  ...
  let n = lstring_of_vinst (snd e) in
    match e with
	  ...
	  save y;
	  [finst3 "store" x spreg (string_of_int (offset y))]
      | NonTail(_), Save(x, y) when List.mem x allfregs && not (S.mem y !stackset) ->
	  savef y;
	  [finst3 "fstore" x spreg (string_of_int (offset y))]
      | NonTail(_), Save(x, y) -> assert (S.mem y !stackset); []
	  (* 復帰の仮想命令の実装 (caml2html: emit_restore) *)
      | NonTail(x), Restore(y) when List.mem x allregs ->
	  [finst3 "load" x spreg (string_of_int (offset y))]
      | NonTail(x), Restore(y) ->
	  assert (List.mem x allfregs);
	  [finst3 "fload" x spreg (string_of_int (offset y))]
	    (* 末尾だったら計算結果を第一レジスタにセットしてret (caml2html: emit_tailret) *)
      | Tail, (Nop | Store _ | Fstore _ | Save _ as exp) ->
	  (g' (NonTail(Id.gentmp Type.Unit), exp)) @ [finst0 "return"]
      | Tail, (Floor _ | Float_of_int _ as exp) ->
	  (g' (NonTail(fregs.(0)), exp)) @ [finst0 "return"]
      | Tail, (Add _ | Sub _ | Mul _ | And _ | Load _ 
	| Or _ | Xor _ | Nor _ | Ori _ | Nori _ | Xori _
	| Andi _ | Addi _ | Subi _ | Muli _ as exp) ->
	  (g' (NonTail(regs.(0)), exp)) @ [finst0 "return"]
	  ...
	  (match locate x with
	     ...
	     | _ -> assert false) @ [finst0 "return"]
      | Tail, IfEq(x, y', e1, e2) ->
	  g'_tail_if e1 e2 x y' "bne"
      | Tail, IfLE(x, y', e1, e2) ->
	  g'_tail_if e1 e2 x y' "bgt"
      | Tail, IfGE(x, y', e1, e2) ->
	  g'_tail_if e1 e2 x y' "blt"
      | Tail, IfFEq(x, y, e1, e2) ->
	  g'_tail_if e1 e2 x y "fbne"
      | Tail, IfFLE(x, y, e1, e2) ->
	  g'_tail_if e1 e2 x y "fbgt"
      | NonTail(z), IfEq(x, y', e1, e2) ->
	  g'_non_tail_if (NonTail(z)) e1 e2 x y' "bne"
      | NonTail(z), IfLE(x, y', e1, e2) ->
	  g'_non_tail_if (NonTail(z)) e1 e2 x y' "bgt"
      | NonTail(z), IfGE(x, y', e1, e2) ->
	  g'_non_tail_if (NonTail(z)) e1 e2 x y' "blt"
      | NonTail(z), IfFEq(x, y, e1, e2) ->
	  g'_non_tail_if (NonTail(z)) e1 e2 x y "fbne"
      | NonTail(z), IfFLE(x, y, e1, e2) ->
	  g'_non_tail_if (NonTail(z)) e1 e2 x y "fbgt"
	    (* 関数呼び出しの仮想命令の実装 (caml2html: emit_call) *)
      | Tail, CallCls(x, ys, zs) -> raise Exit
      | Tail, CallDir(Id.L(x), ys, zs) -> (* 末尾呼び出し *)
	  (g'_args [] ys zs) @ [finst1 "jump" x]
      | NonTail(a), CallCls(x, ys, zs) -> raise Exit
      | NonTail(a), CallDir(Id.L(x), ys, zs) ->
	  let tmp = (g'_args [] ys zs) in
	  let ss = stacksize () in
	    tmp @
	      [finst3 "addi" spreg spreg (string_of_int ss);
	       finst1 "call" x;
	       finst3 "subi" spreg spreg (string_of_int ss)]
	    @
	      (if List.mem a allregs && a <> regs.(0) then
		 [finst3 "add" a zreg regs.(0)]
	       else if List.mem a allfregs && a <> fregs.(0) then
		 [finst3 "fadd" a fzreg fregs.(0)] else [])
...
let f istest memext memin memout memsp memhp floffset (Prog(fl, fundefs, e)) =
  ...
  let ret =
	...
	[
	  ...
	  finst0 "nop";
	  (*スタックポインタ初期化*)
	  finst3 "addi" spreg zreg (string_of_int memsp);
	  (*ヒープポインタ初期化*)
