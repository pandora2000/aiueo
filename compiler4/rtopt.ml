...
let rec trace_reflections index diffuse hilight_scale dirvec =
  ...
  if index >= 0 then (
    ...
    if judge_intersection_fast dvec then
      let surface_id = (imul intersected_object_id.(0) 4) + intsec_rectside.(0) in
...
let rec trace_ray nref energy dirvec pixel dist =
  if nref <= 4 then (
    ...
    (* オブジェクトにぶつかった場合 *)
      ...
      (* pixel tupleに情報を格納する *)
      surface_ids.(nref) <- (imul obj_id 4) + intsec_rectside.(0);
